...
###   bitcoinpaythroughcore.org and bitcoinpaythrough.org and compares them.
...
WORKINGDIR="/tmp/bitcoinpaythrough_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://bitcoinpaythroughcore.org"
HOST2="https://bitcoinpaythrough.org"
BASEDIR="/bin/"
VERSIONPREFIX="bitcoinpaythrough-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'bitcoinpaythrough-',
...
if ! WGETOUT=$(wget -N -O "$SIGNATUREFILENAME.2" "$HOST2$BASEDIR$SIGNATUREFILENAME" 2>&1); then
   echo "bitcoinpaythrough.org failed to provide signature file, but bitcoinpaythroughcore.org did?"
   echo "wget output:"
   # shellcheck disable=SC2001
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "bitcoinpaythrough.org and bitcoinpaythroughcore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Bitcoinpaythrough Core binary release signing key installed?"
   fi
