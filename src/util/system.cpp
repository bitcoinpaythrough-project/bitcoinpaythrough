...
const char * const BITCOINPAYTHROUGH_CONF_FILENAME = "bitcoinpaythrough.conf";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "bitcoinpaythroughd -foo=bar
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //bitcoinpaythrough-tx using stdin
        std::string val;
...
#else
    const char* pszModule = "bitcoinpaythrough";
#endif
    if (pex)
        return strprintf(
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Bitcoinpaythrough
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Bitcoinpaythrough
    // Mac: ~/Library/Application Support/Bitcoinpaythrough
    // Unix: ~/.bitcoinpaythrough
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Bitcoinpaythrough";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Bitcoinpaythrough";
#else
    // Unix
    return pathRet / ".bitcoinpaythrough";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", BITCOINPAYTHROUGH_CONF_FILENAME);
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Make sure Bitcoinpaythrough Core copyright is not removed by accident
    if (copyright_devs.find("Bitcoinpaythrough Core") == std::string::npos) {
