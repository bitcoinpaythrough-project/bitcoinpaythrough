...
## Build Bitcoinpaythrough Core

1. Clone the Bitcoinpaythrough Core source code:
    ```shell
    git clone https://github.com/bitcoinpaythrough/bitcoinpaythrough
    cd bitcoinpaythrough
    ```

2.  Build Bitcoinpaythrough Core:

    Configure and build the headless Bitcoinpaythrough Core binaries as well as the GUI (if Qt is found).
...
## `disable-wallet` mode
When the intention is to run only a P2P node without a wallet, Bitcoinpaythrough Core may be
...
## Running
Bitcoinpaythrough Core is now available at `./src/bitcoinpaythroughd`

Before running, you may create an empty configuration file:
```shell
mkdir -p "/Users/${USER}/Library/Application Support/Bitcoinpaythrough"

touch "/Users/${USER}/Library/Application Support/Bitcoinpaythrough/bitcoinpaythrough.conf"

chmod 600 "/Users/${USER}/Library/Application Support/Bitcoinpaythrough/bitcoinpaythrough.conf"
```

The first time you run bitcoinpaythroughd, it will start downloading the blockchain. This process could
take many hours, or even days on slower than average systems.

You can monitor the download process by looking at the debug.log file:
```shell
tail -f $HOME/Library/Application\ Support/Bitcoinpaythrough/debug.log
```

## Other commands:
```shell
./src/bitcoinpaythroughd -daemon      # Starts the bitcoinpaythrough daemon.
./src/bitcoinpaythrough-cli --help    # Outputs a list of command-line options.
./src/bitcoinpaythrough-cli help      # Outputs a list of RPC commands when the daemon is running.
```

## Notes
* Tested on OS X 10.10 Yosemite through macOS 10.14 Mojave on 64-bit Intel
processors only.
* Building with downloaded Qt binaries is not officially supported. See the notes in [#7714](https://github.com/bitcoinpaythrough/bitcoinpaythrough/issues/7714).
...
- The Apple keyholder uses this unsigned app to create a detached signature, using the
  script that is also included there. Detached signatures are available from this [repository](https://github.com/bitcoinpaythrough-core/bitcoinpaythrough-detached-sigs).
